...
- 불릿 리스트의 시각 시그니처는 "dot only"를 유지:
  - 하이픈처럼 보이는 보조 바를 추가하지 않는다.
- 불릿 중복 표식 금지:
  - 커스텀 불릿(`dot`)이 보이는 라인에서는 원문 마커 글리프(`-`, `*`, `+`)가 추가로 보이면 안 됨.
  - 활성 라인/빈 라인/입력 직후 상태에서도 `dot + '-'` 같은 중복 표시는 릴리즈 차단 사유.
...
- 불릿 마커 시그니처(`dot only`) 유지 확인.
- 불릿 라인에서 중복 마커(`dot` + `-`)가 보이지 않는지 확인.
